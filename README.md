...
2. **打开 VibeShare / Share** — 选文件夹或 HTML，扫码或输入临时访问码。 Pick a folder or HTML file, then scan the QR or enter a temporary code.
...
2. 开始共享，扫码或输入临时访问码。  
   Start sharing, then scan the QR or enter a temporary access code.
3. 同一 Wi-Fi 的设备扫码打开。  
